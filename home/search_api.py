...
def query_problem(kw, all=False):
    ...
    if q:
        for problem in sorted_query(Problem.objects.defer("description").filter(q).distinct().all(), kw):
            results.append(dict(title=escape(problem.title), url=reverse('problem:detail', kwargs=dict(pk=problem.pk))))
...
#             results.append(dict(title=escape(contest.title), url=reverse('contest:dashboard', kwargs={"cid": contest.pk})))
